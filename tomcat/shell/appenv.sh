...
if [ "$JVM_VERSION" \< "1.8" ] ; then
    echo "Require Java 8u40 or later."
    exit 1;
fi
...
# Here we create the arguments that will get passed to the jvm when starting tomcat.

# Read user-defined JVM options from jvm.options file
JVM_OPTS_FILE=$CATALINA_HOME/bin/jvm.options
...
if [ "x$MAX_HEAP_SIZE" = "x" ] && [ "x$HEAP_NEWSIZE" = "x" -o $USING_G1 -eq 0 ]; then
    ...
    echo "please set or unset MAX_HEAP_SIZE and HEAP_NEWSIZE in pairs when using CMS GC (see appenv.sh)"
    exit 1
fi
...
## GC log path has to be defined here because it needs to access CATALINA_HOME
JVM_OPTS="$JVM_OPTS -Xloggc:${CATALINA_HOME}/logs/gc.log"


# Jamm 提供了 MemoryMeter, 一个 java agent 用于测量对象实际消耗内存大小.
# add the jamm javaagent
# JVM_OPTS="$JVM_OPTS -javaagent:$CATALINA_HOME/lib/jamm-0.3.0.jar"

# set jvm HeapDumpPath with CASSANDRA_HEAPDUMP_DIR
if [ "x$CATALINA_HOME" != "x" ]; then
    JVM_OPTS="$JVM_OPTS -XX:HeapDumpPath=$CATALINA_HOME/logs/memory-dump-`date +%s`-pid$$.hprof"
fi
...
#
# To enable remote JMX connections, uncomment lines below(>= 7u4)
#
...
#JVM_OPTS="$JVM_OPTS -Dcassandra.jmx.remote.login.config=CatalinaLogin"
#JVM_OPTS="$JVM_OPTS -Djava.security.auth.login.config=$CATALINA_HOME/conf/catalina-jaas.config"
...
if [ "$REMOTE_CLASS" = "yes" ]; then
    JVM_OPTS="$JVM_OPTS -javaagent:$CATALINA_HOME/jrebel.jar"
    JVM_OPTS="$JVM_OPTS -agentpath:$CATALINA_HOME/libjrebel64.so"
    JVM_OPTS="$JVM_OPTS -Drebel.disable_update=true"
    JVM_OPTS="$JVM_OPTS -Drebel.remoting_plugin=true"
fi


echo "_______________________________________________"
echo ""
echo "java_version: >> $java_ver_output"
#echo "Using JVM_OPTS:"
#for arg in $JVM_OPTS
#do
#    echo ">> " $arg
